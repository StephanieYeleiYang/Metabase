...
async function importFun() {
  try {
    //const IMPORT_HOST = process.env["http://localhost:3000/"];  // for your changing environment like dev , staging , prod

    let jsonObj = await fs.readFileSync("./metabase/data.json", "utf8");
    jsonObj = JSON.parse(jsonObj);

    const destinationUrl = "http://localhost:3000/api/";
    ...
    if (jsonObj.sourceCards) {
      for (const i of jsonObj.sourceCards) {
        i.collection_id = collectionMappingTable.get(i.collection_id);

        if (i.collection_id && i.collection.personal_owner_id == null) {
          const mappingTableValue = destinationCardMappingTable.get(i.name);
          ...
          if (mappingTableValue) {
            ...
            cardMappingTable.set(i.id, updateCard.data.id);
          } else {
            const addedCards = await axios.post(`${destinationUrl}card`, i, {
              headers: {
                "X-Metabase-Session": metabaseSesstion
              }
            });
            // map source cards id with new ceated destination id
            cardMappingTable.set(i.id, addedCards.data.id);
          }
        }
      }
    }
